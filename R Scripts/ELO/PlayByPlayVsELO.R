...
playDist <- new_dat$PlayType[new_dat$posteam == 'WAS'
...
team_cols <- c(nflteams$primary[nflteams$abbr == 'WAS'], 
		nflteams$secondary[nflteams$abbr == 'WAS'], 
		nflteams$tertiary[nflteams$abbr == 'WAS'])

A <- table(playDist)
Apercentlabels <- round(100*A/sum(A), 2)
apielabel <- paste(Apercentlabels, "%", sep = "")
pie(table(playDist), labels = c(apielabel, table(playDist)), main = "Redskins Play Distribution 2018", col = team_cols)
